...
const int fb_cf[FB_NB]=		{40,100,200,320,640,1000,1600,2200,3000,4000,6000,8000,12000,16000};
// Array with the bandwidths of each filter in Hertz
const int fb_bw[FB_NB]=    {30,120,160,320,640,800,1200,1200,1400,1800,2600,4000,6000,8000};
/////////////////////////////////////////////////////////////////
...
void update_eqbank_eq(GtkAdjustment * adj, struct slider_wrapper *p)
{
	p->par->boosts[p->slider_id]=(int)adj->value;
	SetEqBiquad(sample_rate,fb_cf[p->slider_id],fb_bw[p->slider_id],(int)adj->value
		,&p->par->filters[p->slider_id]);
}

void update_eqbank_volume(GtkAdjustment * adj, struct eqbank_params *p)
{
	p->volume=adj->value;
	p->ocoeff=pow(10,p->volume/20.0);
}

void toggle_eqbank(void *bullshit, struct effect *p)
{
    if (p->toggle == 1) {
...
void eqbank_init(struct effect *p)
{
	struct eqbank_params *peq;

	GtkWidget      *boost[FB_NB],*output;
	GtkObject      *adj_boost[FB_NB],*adj_output;
	GtkWidget      *boost_label[FB_NB];
	GtkWidget      *Hzlabel,*Dblabel,*Olabel;
	GtkWidget      *parmTable;
	GtkWidget      *button;
	char s[6];
	int i;
	...
	for (i=0;i<FB_NB;i++)
	{
		...
		gtk_signal_connect(GTK_OBJECT(adj_boost[i]), "value_changed",
		       GTK_SIGNAL_FUNC(update_eqbank_eq), (void*)&sl_wrappers[i]);
		gtk_table_attach(GTK_TABLE(parmTable), boost[i], i+1, i+2, 1, 2,
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK),
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK), 3, 3);

//		itoa(fb_cf[i],s,10);  // Convert the center frequencies to nb for display
#ifndef _WIN32		
		...
		gtk_table_attach(GTK_TABLE(parmTable), boost_label[i], i+1, i+2, 0, 1,
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK),
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK), 3, 3);

	}
	Olabel = gtk_label_new("Output\n Volume (dB)");
	gtk_table_attach(GTK_TABLE(parmTable), Olabel, FB_NB+1, FB_NB+2, 0, 1,
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK),
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK), 3, 3);
	...
	gtk_signal_connect(GTK_OBJECT(adj_output), "value_changed",
		       GTK_SIGNAL_FUNC(update_eqbank_volume), (void*)peq);
	gtk_table_attach(GTK_TABLE(parmTable), output, FB_NB+1, FB_NB+2, 1, 2,
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK),
					__GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND |	GTK_SHRINK), 3, 3);
    ...
    gtk_table_attach(GTK_TABLE(parmTable), button, FB_NB>>1, (FB_NB>>1)+1, 2, 3,
		     __GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND | GTK_SHRINK),
		     __GTKATTACHOPTIONS(GTK_FILL | GTK_EXPAND | GTK_SHRINK), 0, 0);
    ...
    gtk_window_set_title(GTK_WINDOW(p->control), (gchar *) ("Equalizers Bank"));
...
void	eqbank_done(struct effect *p)
{
	//int i;
	struct eqbank_params *ep;
	...
	free(ep->filters); free(ep->boosts);
...
void	eqbank_save(struct effect *p, int fd)
{
	struct eqbank_params *ep;
...
void	eqbank_load(struct effect *p, int fd)
{
	struct eqbank_params *ep;
	int i;
...
void	eqbank_create(struct effect *p)
{
	struct eqbank_params *peq;
	int i;
	...
	p->params = (struct eqbank_params *) malloc(sizeof(struct eqbank_params));
	peq=(struct eqbank_params *) p->params;
