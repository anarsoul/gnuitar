...
void CalcChebyshev2(double Fs,double Fc,double ripple, int lowpass, struct Biquad *filter)
{
double x,y,z,c,v,t,r,om,m,x0,x1,x2,y1p,y2,k,d,tt,tt2;
int i;
	//x=-cos(PI/4+PI/2);
	//y=sin(PI/4+PI/2);
	...
	y1p=(8-2*m*tt2)/d;
	y2=(-4-4*c*tt-m*tt2)/d;
	if (lowpass)  k=sin(0.5-om/2)/sin(0.5+om/2);
	else k=-cos(om/2+0.5)/cos(om/2-0.5);
	d=1+k*(y1p-y2*k);
